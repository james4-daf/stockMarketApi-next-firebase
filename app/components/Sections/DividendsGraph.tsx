...
// Function to calculate the average percentage increase over n years
    const calculateAvgIncrease = (data: Array<{ year: number, dividend: number }>, years: number) => {
        ...
        for (let i = 1; i < years; i++) {
            ...
            const percentageIncrease = ((currentYear.dividend - previousYear.dividend) / previousYear.dividend) * 100;
...
// Function to calculate total dividends for n years
    const calculateTotalPercentageGain = (data: Array<{ year: number, dividend: number }>, years: number) => {
        if (data.length < years) return null;
        ...
        const percentageGain = ((lastYearData.dividend - firstYearData.dividend) / firstYearData.dividend) * 100;
    ...
    return (
        <>
                ...
                <CardContent className="px-2 sm:p-6">
                    <ChartContainer config={chartConfig} className="aspect-auto h-[250px] w-full">
                        ...
                        >
                            ...
                            <ChartTooltip
                                content={
                                    <ChartTooltipContent className="w-[150px]" labelFormatter={(_, payload) => {
            ...
            <div className="px-6 py-4 flex">
                ...
                <div>
                <h2 className="text-lg font-semibold mt-4 mb-2">Dividend Increase Summary</h2>
                <ul className="space-y-1">
                    {[
                        ...
                        <li key={years} className="text-sm">
                            ...
                            Avg Increase: {avg ? avg.toFixed(1) + "%" : "N/A"},
                            Total Gain: {total ? total.toFixed(2) + "%" : "N/A"}
                        </li>
                    ))}
                </ul>
                </div>
